...
支持输入课件和 PDF 文档<br>
支持输出 **速查表 (cheatsheet) / 讲义 / 教材 / 习题与答案**，格式可为 **Markdown / Word DOCX / LaTeX**<br>
...
- 支持单文件或文件夹批量输入（PPT/PPTX/PDF）
- 速查表模式优先覆盖难题公式与高频考点
- 支持教师讲义、教材正文、复习笔记等多种形态
- 支持“习题与答案”输出
- LaTeX 输出自动编译 PDF
- DOCX 与 LaTeX 保留公式与符号
...
- TeX Live（或 `xelatex` 等引擎），用于将 LaTeX 编译成 PDF
...
1. 使用者用自然语言提出需求。
2. 如果信息不够，Agent 主动追问。
3. 提取脚本读取 PPT / PDF（或文件夹中的所有文件）。
4. Agent 以提取结果为依据。
5. Agent 使用自己的 LLM 写作能力生成速查表、讲义或教材正文等最终输出。
6. 最终保存为 Markdown、LaTeX（并编译 PDF），或导出为 DOCX。

如果输出模式是速查表，Agent 必须严格控制页数，把它当作硬性约束。排版要紧凑，尽量少留空白。
...
- 请使用 `slides2anything` 根据我的课件生成“习题与答案”。
...
- 想要速查表、教师讲义、教材正文、习题与答案、笔记还是学生讲义
- 如果是速查表，大概希望控制在多少页
- 是否需要学习目标、术语表、小结和练习题
...
- 如果输入是文件夹，提取器会读取目录中的所有支持文件并综合生成输出。
- 对 LaTeX 输出，Agent 应使用 `xelatex` 等引擎编译生成 PDF。
