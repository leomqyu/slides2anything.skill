...
**slides2anything** 是一个把 `.ppt`、`.pptx`、`.pdf` 内容整理成高价值学习材料的 skill。  
它不只是提取文字，更重要的是让 Agent 先补问信息、读取原文，再用自己的 LLM 能力把内容写成更适合考试复习、课堂教学和教材整理的文本。
...
|------|------|
| 只负责提取文字，最后给你一堆碎片 | 先提取，再帮助 Agent 把碎片内容写成可教学的章节 |
| 默认你第一次提问就把所有参数说全 | Agent 会主动追问路径、格式、模式和结构要求 |
| 把 PPT 和 PDF 当成普通文本文件处理 | 把它们当成有教学结构的课程材料来理解 |
| 容易生成扁平笔记或僵硬模板 | 借助模型写出速查表、教师讲义、公式总结和完整教材内容 |
| 只是导出一个文件 | 更强调生成一个真的能继续教学、编辑和出版的初稿 |

一句话说，这个项目的重点是 **面向教学与考试的改写**，而不只是文本提取。
...
3. 提取脚本读取 PPT 或 PDF。
4. Agent 以提取结果为依据。
5. Agent 使用自己的 LLM 写作能力生成速查表、讲义或教材正文。
...
- 请使用 `slides2anything` 把我的课件转成 DOCX 或者 LaTeX 教材初稿，一步步问我需要确认的选项。
...
- 这个项目的目标是生成高质量初稿，不是凭空补造源文件里根本不存在的知识细节。
